...
//#define USE_WIFI_MODULE_AS_HTTP_PROVIDER // закомментировать, если не хотим посылать HTTP-запрсы к сервису gardenboss.ru через ESP
...
//#define USE_GSM_MODULE_AS_HTTP_PROVIDER // закомментировать, если не хотим посылать HTTP-запрсы к сервису gardenboss.ru через GSM
...
//#define USE_HTTP_MODULE // закомментировать, если не нужна проверка на входящие команды (с сервиса gardenboss.ru)
//--------------------------------------------------------------------------------------------------------------------------------
// расширители портов
//--------------------------------------------------------------------------------------------------------------------------------
...
#define UNI_WIRED_MODULES A4// номера пинов (через запятую), на которых висят универсальные модули, кол-вом  UNI_WIRED_MODULES_COUNT
...
#define RS_485_TXC UART_SR_TXRDY // бит ТХ, связанный с номером UART RS_485_SERIAL
...
//--------------------------------------------------------------------------------------------------------------------------------
#define EEPROM_USED_MEMORY EEPROM_AT24C64 // по умолчанию используем встроенную в мегу память (типы памяти см. в Configaration_Shared.h)
//--------------------------------------------------------------------------------------------------------------------------------
...
#define PH_SENSOR_PIN A3 // номер аналогового пина, с которого читать показания датчика (0 - нет датчика, прикреплённого к меге)
...
#define SOIL_MOISTURE_SENSORS {A2, ANALOG_SOIL_MOISTURE}//,{A2, FREQUENCY_SOIL_MOISTURE} // АНАЛОГОВЫЕ пины, на которых сидят датчики влажности почвы (через запятую, кол-во равно SUPPORTED_SOIL_MOISTURE_SENSORS!)
...
#define WIFI_AVAILABLE_CHECK_TIME 30000 // через сколько миллисекунд проверять доступность ESP посылкой команды AT
