...
//--------------------------------------------------------------------------------------------------------------------------------
#define SDCARD_CS_PIN 50 // номер пина Chip Select для SD-модуля 
#define VIRTUAL_PIN_START_NUMBER 55 //80 // номер пина, с которого все пины будут считаться виртуальными, т.е. запись в них производиться не будет, однако в карте пинов (16 байт) этот статус будет отражён.
...
#define HUMIDITY_SENSORS H_SENSOR(14,0,DHT2x), H_SENSOR(15,0,DHT2x)
...
#define TEMP_SENSORS_PINS ADD_T(22,DS18B20)

#define SUPPORTED_WINDOWS 2 // кол-во поддерживаемых окон, максимум 16 (по два реле на мотор, для 8-ми канального модуля реле - 4 окна)
...
#define WINDOWS_RELAYS_PINS 26,27,28,29
...
#define MENU_BUTTON_PIN A2 // пин, на котором висит кнопка перехода внутрь экрана и выбора между параметрами
...
#define ENCODER_PULSES_PER_CLICK 16 // сколько импульсов приходится на один клик энкодера, подбирать опытным путём, для удобства
...
#define WAIT_SCREEN_SENSORS W_S(StateTemperature,0,"STATE","T внутри") ,W_S(StateHumidity,0,"HUMIDITY","Влажность - верх"), W_S(StateHumidity,1,"HUMIDITY","Влажность - низ")
